 ...
 * 
 * modified slightly to accept a stream instead of a file
 * and not have any notion of FileName - DM
   ---------------------------------------------------------- */
...
namespace MusicCompany.Core.Services.AudioConversion
{
	public class MP3Header
	{
		...
		public bool ReadMP3Information(Stream mp3Stream, bool leaveOpen)
		{
			
			// Set the file size
			lngFileSize = mp3Stream.Length;
			...
			do
			{
				mp3Stream.Position = intPos;
				mp3Stream.Read(bytHeader, 0, 4);
				intPos++;
				LoadMP3Header(bytHeader);
			}
			while (!IsValidHeader() && (mp3Stream.Position != mp3Stream.Length));
			...
			if (mp3Stream.Position != mp3Stream.Length)
			{
				...
				mp3Stream.Position = intPos;
				mp3Stream.Read(bytVBitRate, 0, 12);
